...
self.onmessage = (ev) => {
  const { id, flower, split, hat, placement } = ev.data;
  try {
    ...
    if (out.hat) transfer.push(out.hat.pos.buffer, out.hat.flowerIdx.buffer,
                               out.hat.wallIdx.buffer);
    self.postMessage({ id, type: 'done', ...out }, transfer);
  } catch (err) {
